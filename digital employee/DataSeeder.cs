...
namespace digital_employee
{
    public static class DataSeeder
    {
        ...
        private static async Task<string?> SeedBusinessAsync(
            ...
            BusinessSeed seed)
        {
            // If owner already exists, top up customers only then return
            var existingUser = await userManager.FindByEmailAsync(seed.OwnerEmail);
        ...
        private static async Task SeedMissingCustomersAsync(
            AppDbContext context, Business business, BusinessSeed seed)
        {
            var allCustomerNames = new[]
            {
                "Ahmed Hassan",   "Sara Mohamed",   "Omar Khaled",
            ...
            var rng           = new Random(business.Id.GetHashCode());
            int targetCount   = rng.Next(10, 16);
            var emailTag      = seed.Name.Split('|')[0].Trim().Replace(" ", "").ToLower();

            // Find how many customers already exist for this business
            var existingEmails = await context.Customers
                .Where(c => c.BusinessId == business.Id)
                .Select(c => c.Email)
                .ToListAsync();

            int existingCount = existingEmails.Count;
            if (existingCount >= targetCount) return; // nothing to add

            var menuItems = await context.MenuItems
                .Where(m => m.BusinessId == business.Id && m.IsAvailable)
                .ToListAsync();
            ...
            var ticketTemplates = new[]
            {
                new { Subject = "الأوردر اتأخر كتير | Order very late", TicketType = "LateDelivery", Priority = "High", Resolution = "تم التواصل مع المندوب وتم التوصيل. اعتذرنا وقدمنا كوبون خصم. | Rider contacted, order delivered. Apology and discount coupon issued.", Opening = "الأوردر بتاعي اتأخر أكتر من ساعة! | My order is over an hour late!", AiAck = "معلش جداً على التأخير. ممكن تقولي رقم الأوردر؟ | So sorry for the delay. Can you share your order number?", CustomerReply = "الرقم مش معايا بس طلبته من فترة | I don't have it but I ordered a while ago", AiInvestigate = "بشوف الأوردر... في تأخير غير متوقع بسبب ضغط الطلبات. بترفع الموضوع للدعم فوراً. | Checking now... unexpected delay due to high demand. Escalating right away.", AiEscalate = "تم فتح تذكرة دعم أولوية عالية. فريق الدعم هيتواصل معاك خلال 15 دقيقة. | High-priority ticket created. Support team will contact you within 15 minutes.", CustomerClose = "تمام، شكراً | Okay, thanks", FeedbackRating = 3, FeedbackComment = "اتأخر التوصيل بس اتحل | Delivery was late but resolved" },
                new { Subject = "الأوردر وصل ناقص | Order arrived incomplete", TicketType = "MissingItem", Priority = "Normal", Resolution = "تم إرسال الأيتم الناقص مجاناً في توصيلة منفصلة. | Missing item was sent in a separate free delivery.", Opening = "الأوردر وصل بس فيه صنف ناقص! | My order arrived but something is missing!", AiAck = "معلش! إيه الصنف اللي مجاش؟ | Sorry! What item was missing?", CustomerReply = "الطبق الجانبي اللي طلبته مجاش مع الأوردر | The side dish I ordered didn't come", AiInvestigate = "آسف جداً على الإزعاج. براجع الأوردر دلوقتي... أيوه، في خطأ من جانبنا. | So sorry for the inconvenience. Reviewing your order now... yes, there was a mistake on our end.", AiEscalate = "هنبعتلك الصنف الناقص في أقرب وقت مجاناً. فتحت تذكرة علشان نتابع. | We'll send the missing item to you for free ASAP. Ticket created to follow up.", CustomerClose = "تمام جزاكم الله خير | Thank you so much", FeedbackRating = 4, FeedbackComment = "حلوا المشكلة بسرعة وعوضوني | They solved it quickly and compensated me" },
                new { Subject = "الأكل وصل بارد | Food arrived cold", TicketType = "FoodQuality", Priority = "Normal", Resolution = "تم تقديم اعتذار وإرسال خصم 20% على الطلب القادم. | Apology given and 20% discount on next order.", Opening = "الأكل وصلني بارد خالص، مش مقبول! | The food arrived completely cold, unacceptable!", AiAck = "معلش جداً! الأكل لازم يوصل ساخن. ممكن تقولي إيه الأصناف اللي وصلت باردة؟ | I'm so sorry! Food should arrive hot. Which items were cold?", CustomerReply = "كل حاجة باردة، بدو إنه اتأخر كتير في الطريق | Everything was cold, seems it took too long on the way", AiInvestigate = "هذا مقبولش. بشوف المدة اللي أخدها التوصيل... فعلاً اتأخر أكتر من المعتاد. | That's not acceptable. Checking the delivery time... it was indeed longer than usual.", AiEscalate = "بفتح تذكرة لفريق الجودة وهتواصل معاك فريق الدعم. هنعوضك على الإزعاج. | Opening a quality ticket and support will contact you. We'll compensate for the inconvenience.", CustomerClose = "شكراً، استنى تواصلهم | Thanks, awaiting their call", FeedbackRating = 3, FeedbackComment = "المشكلة اتحلت بعد التواصل | Issue resolved after follow-up" },
                new { Subject = "طلب إلغاء الأوردر | Order cancellation request", TicketType = "Cancellation", Priority = "Normal", Resolution = "تم إلغاء الأوردر واسترجاع المبلغ خلال 24 ساعة. | Order cancelled and refund processed within 24 hours.", Opening = "عايز ألغي الأوردر بتاعي، ممكن؟ | I want to cancel my order, is that possible?", AiAck = "أيوه ممكن نلغي لو لسه ما اتجهزش. ممكن تقولي رقم الأوردر؟ | Yes, we can cancel if it hasn't been prepared yet. Can I have your order number?", CustomerReply = "مش معايا الرقم بس طلبته من شوية بس | I don't have the number but I just ordered", AiInvestigate = "بشوف آخر أوردر عليك... لقيته، الأوردر لسه في مرحلة التجهيز. | Checking your latest order... found it. The order is still being prepared.", AiEscalate = "بفتح طلب إلغاء وبرفعه للمطبخ فوراً. هيتواصل معاك فريق الدعم للتأكيد وإرجاع الفلوس لو دفعت. | Cancellation request submitted to kitchen immediately. Support will contact you to confirm and process the refund.", CustomerClose = "شكراً جزيلاً | Thank you very much", FeedbackRating = 5, FeedbackComment = "خدمة ممتازة وسريعة | Excellent and fast service" },
                new { Subject = "الأوردر وصل غلط | Wrong order delivered", TicketType = "WrongItem", Priority = "High", Resolution = "تم إرسال الأوردر الصحيح واسترجاع الغلط. | Correct order sent and wrong order retrieved.", Opening = "جالي أوردر غلط! ده مش اللي طلبته | I received the wrong order! This isn't what I ordered", AiAck = "معلش جداً على الإزعاج! إيه اللي وصلك وإيه اللي كنت طالبه؟ | So sorry! What did you receive and what did you order?", CustomerReply = "طلبت وجبة دجاج وجالي سمك! | I ordered chicken and received fish!", AiInvestigate = "ده خطأ كبير وأنا آسف عليه. بترفع الأمر فوراً لفريق التوصيل والمطبخ. | This is a serious mistake and I apologize. Escalating immediately to delivery and kitchen teams.", AiEscalate = "تم فتح تذكرة طارئة. هيبعتوا الأوردر الصح في أقرب وقت مجاناً ويستلموا الغلط. | Emergency ticket opened. Correct order will be sent ASAP for free and wrong order will be picked up.", CustomerClose = "تمام، استنى | Okay, I'll wait", FeedbackRating = 4, FeedbackComment = "المشكلة اتحلت بسرعة ونفس اليوم | Resolved quickly the same day" },
            };

            // Add only the missing customers (starting from existingCount index)
            for (int ci = existingCount; ci < targetCount; ci++)
            {
                var prefix = ci % 3 == 0 ? "010" : ci % 3 == 1 ? "011" : "012";
                var customer = new Customer
                {
                    CustomerId = Guid.NewGuid().ToString(),
                    FullName   = allCustomerNames[ci],
                    Email      = $"customer{ci + 1}@{emailTag}.com",
                    Phone      = $"{prefix}{(10000000 + ci * 1111111) % 100000000:D8}",
                    BusinessId = business.Id,
                    CreatedAt  = DateTime.UtcNow.AddDays(-(60 - ci * 4))
                };
